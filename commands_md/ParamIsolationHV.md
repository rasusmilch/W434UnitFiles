...
- `DIdtTimeThreshold`: `ttime=PARAM_DontChange` — Time threshold for teh dIdt detector which must not be exceeded with a current wihich ist greater than the current threshold.
...
; If this parameter is enabled the test system always executes an required automatic range switching of the measuring devices.; Allowed values: <ResistanceValue>, OFF

## Example

```ats
ParamIsolationHV(600V, 800MOhm, 2s, PARAM_DontChange, 2s, ON, OFF, 1mA, 1, 1000Vpms);
```
...
`IsolationTestHV`, `NoConnAllHV`, `NoConnectionHV`, `NoConnGroupHV`, `ParamContinuity`, `ParamDielectricBreakdown`, `ParamGetIsolationHV`, `ParamIsolationLV`
