# TCPReceiveBinaryData

## Declaration

```ats
function TCPReceiveBinaryData(Handle: integer; Data: tcreatearray; MinBytes: integer = 1): integer;
```

## Call pattern

```ats
TCPReceiveBinaryData(Handle, Data);
```
...
- `Data`: `tcreatearray` — Variable which returns the received data
- `MinBytes`: `integer = 1` — If this number of characters is received the reception will be aborted.

## Return value

The function returns the number of the received bytes.
