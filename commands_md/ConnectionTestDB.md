...
```ats
function ConnectionTestDB(Name: string; Pin1: tpin; Pin2: tpin): boolean;
```

## Call pattern

```ats
ConnectionTestDB('Name', "Pin1", "Pin2");
```

## Description

Tests with the parameters of the dielectric breakdown test whether there is a connection between Pin1 and Pin2.
...
```ats
ConnectionTestDB('ConnectionTestDB1', "Pin1", "Pin2");
```
...
`ConnectionTest`, `ConnectionTestHV`, `ConnectionTestLV`, `ParamDielectricBreakdown`
