...
```ats
function NoConnLowerLV(Name: string; Pin: integer): boolean;
```

## Call pattern

```ats
NoConnLowerLV('Name', "Pin");
```

## Description

Tests with low voltage whether Pin is not connected to any other pin with a lower address
...
`IsolationTestLV`, `NoConnAllLV`, `NoConnLowerDB`, `NoConnLowerHV`
