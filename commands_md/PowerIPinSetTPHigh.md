...
```ats
function PowerIPinSetTPHigh(ExternalIO: integer; Pin: tpin): void;
```

## Call pattern

```ats
PowerIPinSetTPHigh(EXTIO_I?, "Pin");
```

## Description

Connects the power I - pin that is assigned to "Pin" to the high potential of "ExternalIO".
...
`PowerIPinSetHigh`, `PowerIPinSetTPLow`, `PowerIPinSetTPOff`
