...
```ats
function NoConnAllHV(Name: string; Pin: tpin): boolean;
```

## Call pattern

```ats
NoConnAllHV('Name', "Pin");
```

## Description

Tests with high voltage whether Pin is not connected to any other pin.
...
`IsolationTestHV`, `NoConnAllDB`, `NoConnAllLV`, `ParamIsolationHV`
