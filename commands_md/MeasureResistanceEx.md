...
```ats
function MeasureResistanceEx(Pin1: tpin; Pin2: tpin; Parameters: tarray; Results: tcreatearray): boolean;
```

## Call pattern

```ats
MeasureResistanceEx("Pin1", "Pin2", Parameters, Results);
```
...
Valid = MeasureResistanceEx("1", "2", Parameters, ResultValues);
if (Valid)
begin
...
`CreateMeasurementParameterList`, `MeasureRLCEx`, `MeasureVoltageEx`
