...
```ats
function MeasureRLC(Name: string; Pin1: tpin; Pin2: tpin; ResultValues: tcreatearray): boolean; tests rlccombinations;
```

## Call pattern

```ats
MeasureRLC('Name', "Pin1", "Pin2", ResultValues);
```

## Description

Measures a RLC combination between Pin1 and Pin2. Creates a list "Values" and fills it with the measured values.
...
`AttenuatorTest`, `CapacitorTest`, `CTwistTestAC`, `DiodeTest`, `MeasureRLCCustom`, `MeasureRLCEx`, `ResistorTest`, `RLCDissipationTest`, `RLCImpedanceAbsoluteTest`, `RLCImpedanceImaginaryTest`, `RLCImpedanceRealTest`, `RLCParallelCapacitanceTest`, `RLCParallelInductanceTest`, `RLCParallelResistanceTest`, `RLCPhaseAngleTest`, `RLCQualityTest`, `RLCSerialCapacitanceTest`, `RLCSerialInductanceTest`, `RLCSerialResistanceTest`, `ZDiodeTest`
