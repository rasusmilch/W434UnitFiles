...
```ats
function TCPSendString(Handle: integer; Data: string): boolean;
```

## Call pattern

```ats
TCPSendString(Handle, 'Data');
```

## Description

The function sends a string over the TCP/IP connection which is specified by the passed value.
...
`TCPConnect`, `TCPDisconnect`, `TCPGetError`, `TCPPing`, `TCPReceiveString`, `TCPSendBinaryData`
