...
```ats
function PowerIPinSetTPOff(ExternalIO: integer; Pin: tpin): void;
```

## Call pattern

```ats
PowerIPinSetTPOff(EXTIO_I?, "Pin");
```

## Description

Disconnects the power I - pin that is assigned to "Pin" from high and low potential of ExternalIO.
...
`PowerIPinSetOff`, `PowerIPinSetTPHigh`, `PowerIPinSetTPLow`, `PowerPinResetAll`
