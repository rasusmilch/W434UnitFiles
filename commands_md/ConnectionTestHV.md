...
```ats
function ConnectionTestHV(Name: string; Pin1: tpin; Pin2: tpin): boolean;
```

## Call pattern

```ats
ConnectionTestHV('Name', "Pin1", "Pin2");
```

## Description

Tests with the parameters of the HV isolation test whether there is a connection between Pin1 and Pin2.
...
```ats
ConnectionTestHV('ConnectionTestHV1', "Pin1", "Pin2");
```
...
`ConnectionTest`, `ConnectionTestDB`, `ConnectionTestLV`, `ParamIsolationHV`
