...
```ats
function NoConnLowerHVCombi(Name: string; Pin: tpin): boolean;
```

## Call pattern

```ats
NoConnLowerHVCombi('Name', "Pin");
```

## Description

Tests with high direct and alternating voltage whether the insulation between Pin and all pins with lower address corresponds to the specified parameters.
...
`IsolationTestHVCombi`, `NoConnAllHVCombi`, `NoConnectionHVCombi`, `NoConnGroupHVCombi`, `ParamDielectricBreakdown`, `ParamIsolationHV`, `ParamIsolationHVCombi`
