...
```ats
function InductorTest(Name: string; Pin1: tpin; Pin2: tpin): boolean; tests inductors;
```

## Call pattern

```ats
InductorTest('Name', "Pin1", "Pin2");
```

## Description

Tests an inductor between Pin1 and Pin2 with DC.

The function always refers to an inductor in the netlist.
...
```ats
InductorTest('Inductor1', "Pin1", "Pin2");
```
...
`AttenuatorTest`, `CapacitorTest`, `CTwistTestAC`, `DiodeTest`, `MeasureRLC`, `ResistorTest`, `ZDiodeTest`
