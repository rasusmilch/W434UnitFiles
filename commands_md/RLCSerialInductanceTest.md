# RLCSerialInductanceTest

## Declaration

```ats
function RLCSerialInductanceTest(Name: string; Pin1: tpin; Pin2: tpin; Minimum: tinductance; Maximum: tinductance): boolean; tests rlccombinations;
```

## Call pattern

```ats
RLCSerialInductanceTest('Name', "Pin1", "Pin2", <Minimum>mH, <Maximum>mH);
```

## Description

The function tests the inductance of a RLC combination.
...
```ats
RLCSerialInductanceTest('RLC 1 Lser', "Pin1", "Pin2", 40mH, 60mH);
```
...
| `RES_Lmin` | `real` | Parameter: Minimum inductance in Henry |
| `RES_Lmax` | `real` | Parameter: Maximum inductance in Henry |
...
`MeasureRLC`, `RLCDissipationTest`, `RLCImpedanceAbsoluteTest`, `RLCImpedanceImaginaryTest`, `RLCImpedanceRealTest`, `RLCParallelInductanceTest`, `RLCPhaseAngleTest`, `RLCQualityTest`, `RLCSerialCapacitanceTest`, `RLCSerialResistanceTest`
