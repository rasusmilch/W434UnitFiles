...
```ats
function NoConnLowerDB(Name: string; Pin: tpin): boolean;
```

## Call pattern

```ats
NoConnLowerDB('Name', "Pin");
```

## Description

Tests with alternating voltage whether Pin is not connected to any other pin with a lower address
...
`DielectricBreakdownTest`, `NoConnAllDB`, `NoConnLowerHV`, `NoConnLowerLV`
