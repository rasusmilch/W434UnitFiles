# UIAutostartSetString

## Declaration

```ats
function UIAutostartSetString(Commands: integer; Property: string; Value: string): void;
```

## Call pattern

```ats
UIAutostartSetString(COMMANDS_?, 'Property', 'Value');
```

## Description

Sets the value of a string property of an Autostart window.
...
- `Property`: `string` — Allowed values: 'Text', 'File'
- `Value`: `string`
...
`ParamAutostart`, `UIAutostartSelectWindowType`, `UIAutostartSetBoolean`, `UIAutostartSetColor`, `UIAutostartSetWindowPosition`
