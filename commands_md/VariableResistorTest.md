...
```ats
function VariableResistorTest(Name: string; Pin1: tpin; Pin2: tpin): boolean; tests variable_resistors;
```

## Call pattern

```ats
VariableResistorTest('Name', "Pin1", "Pin2");
```

## Description

Tests a variable resistor between Pin1 and Pin2.
...
```ats
VariableResistorTest('VariableResistor1', "Pin1", "Pin2");
```
...
| `RES_StateName` | `string` | Tested state |
| `RES_StateIndex` | `string` | Index of the tested state |
...
`NWSetVariableResistorState`, `ResistorTest`, `ResistorTestCustom`
