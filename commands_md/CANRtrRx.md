...
```ats
function CANRtrRx(Name: string; Controller: integer; RequestedIdentifier: integer; RequestedDataLength: integer; ExpectedIdentifiers: tintegerarray; Period: ttime; ReceivedData: tcreatearray; Text: string = ''; File: string = ''): integer;
```

## Call pattern

```ats
CANRtrRx('Name', <Controller 1-8>, $<RequestedIdentifier>, <RequestedDataLength>, [$<RxIdentifier1>, $<RxIdentifier2>...], <Period>ms, ReceivedData, 'My text', '.\Images\MyFile.jpg');
```

## Description

The function sends a remote frame on the CAN bus. Afterwards it receives data frames for a specified period of time.
...
- `Controller`: `integer` — Number of the CAN controller
- `RequestedIdentifier`: `integer` — Requested identifier
- `RequestedDataLength`: `integer` — Requested data length
...
```ats
ReceivedFrames = CANRtrRx('My Tx/Rx', 1, $100, 2, [$100], 5s, ReceivedData);
if (ReceivedFrames == 0)
begin
...
begin
   ...
   begin
      ...
      Line = StrAdd('CANRtrRx ', Identifier);
      Line = StrAdd(Line, ' ');
...
In this example the identifier $100 is requested with two bytes of data.
Afterwards the function waits for 5s for data. If data frames with the identifier $100 are received during this period they will be displayed on the screen.

## See also

`CANRtr`, `CANRtrRxTest`, `CANTxRx`
