...
```ats
function TCPSetTimeout(Handle: integer; Timeout: ttime): void;
```

## Call pattern

```ats
TCPSetTimeout(Handle, <Timeout>s);
```

## Description

With this function a timeout can be set for the TCP/IP connection which ist specified by the value in Handle.
This is the maximum time for which the function TCPReceiveString receives data.
