...
```ats
function CTwistTestAC(Name: string; Pin1: tpin; Pin2: tpin): boolean; tests ctwists;
```

## Call pattern

```ats
CTwistTestAC('Name', "Pin1", "Pin2");
```

## Description

Tests a C-twist between Pin1 and Pin2 with AC.
...
`AttenuatorTest`, `CapacitorTest`, `DiodeTest`, `MeasureRLC`, `ResistorTest`, `TestCTwistsAC`, `ZDiodeTest`
