...
```ats
function PowerMPinSetTPOff(ExternalIO: integer; Pin: tpin): void;
```

## Call pattern

```ats
PowerMPinSetTPOff(EXTIO_M?, "Pin");
```

## Description

Disconnects the power M - pin that is assigned to "Pin" from high and low potential of ExternalIO.
...
`PowerMPinSetOff`, `PowerMPinSetTPHigh`, `PowerMPinSetTPLow`, `PowerPinResetAll`
