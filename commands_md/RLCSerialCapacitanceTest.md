# RLCSerialCapacitanceTest

## Declaration

```ats
function RLCSerialCapacitanceTest(Name: string; Pin1: tpin; Pin2: tpin; Minimum: tcapacitance; Maximum: tcapacitance): boolean; tests rlccombinations;
```

## Call pattern

```ats
RLCSerialCapacitanceTest('Name', "Pin1", "Pin2", <Minimum>uF, <Maximum>uF);
```
...
```ats
RLCSerialCapacitanceTest('RLC 1 Cser', "Pin1", "Pin2", 8nF, 12nF);
```
...
`MeasureRLC`, `RLCDissipationTest`, `RLCImpedanceAbsoluteTest`, `RLCImpedanceImaginaryTest`, `RLCImpedanceRealTest`, `RLCParallelCapacitanceTest`, `RLCPhaseAngleTest`, `RLCQualityTest`, `RLCSerialInductanceTest`, `RLCSerialResistanceTest`
