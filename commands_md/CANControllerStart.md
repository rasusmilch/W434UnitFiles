...
```ats
function CANControllerStart(Controller: integer): boolean;
```

## Call pattern

```ats
CANControllerStart(<Controller>);
```

## Description

Starts a CAN controller. After the start data can be sent and received.
...
`CANControllerInit`, `CANControllerSetAcceptanceFilter`, `CANControllerStop`, `CANGetError`, `CANISO15765TxRxTest`, `CANRxTest`, `CANTx`, `CANTxRxTest`
