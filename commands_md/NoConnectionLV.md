...
```ats
function NoConnectionLV(Name: string; Pin1: tpin; Pin2: tpin): boolean;
```

## Call pattern

```ats
NoConnectionLV('Name', "Pin1", "Pin2");
```

## Description

Tests with the paramerers of the LV isolation test whether Pin1 and Pin2 are not connected with each other.
...
```ats
NoConnectionLV('NoConnectionLV1', "Pin1", "Pin2");
```
...
`IsolationTestLV`, `NoConnAllLV`, `NoConnectionDB`, `NoConnectionHV`, `NoConnGroupLV`, `ParamIsolationLV`
