...
```ats
function WireTest(Name: string; Pin1: tpin; Pin2: tpin): integer; tests wires;
```

## Call pattern

```ats
WireTest('Name', "Pin1", "Pin2");
```
...
| `RES_AutoStartPassedAfterFail` | `boolean` | TRUE, if the exactly metered value ist higher than the threshold after an error was detected. |

## See also

`ConnectionTest`, `IsConnected`, `NetworkTest`, `ParamAutostart`, `ParamCheckForInterchangedWires`, `ParamContinuity`, `ParamStopOnFail`, `TestWires`
