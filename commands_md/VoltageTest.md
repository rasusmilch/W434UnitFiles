...
```ats
function VoltageTest(Name: string; Pin1: tpin; Pin2: tpin; Value: tvoltage; LowerTol: tvoltage; UpperTol: tvoltage): boolean;
```

## Call pattern

```ats
VoltageTest('Name', "Pin1", "Pin2", <Value>V, <LowerTol>V, <UpperTol>V);
```

## Description

Tests a voltage between Pin1 and Pin2.
...
```ats
VoltageTest('Voltage1', "Pin1", "Pin2", 10V, 2V, 2V);
```
...
`VoltageFrequencyTest`, `VoltageTestCustom`
