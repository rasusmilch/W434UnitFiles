...
```ats
function TwistedPairTest(Name: string; Category: integer; IsShieldTestEnabled: boolean; AdapterName: string = 'Default'; CableLength: tlength = 0m; LowerTol: tlength = 0m; UpperTol: tlength = 0m): boolean;
```

## Call pattern

```ats
TwistedPairTest('Name', TWISTED_PAIR_CAT?, ON);
```
...
- `LowerTol`: `tlength = 0m` — Lower tolerance for the cable lenght.
- `UpperTol`: `tlength = 0m` — Upper tolerance for the cable lenght.
...
`TwistedPairTestFrequencies`, `TwistedPairRunCompensation`, `TwistedPairTest2PE`, `ParamTwistedPair`, `ParamStopOnFail`
