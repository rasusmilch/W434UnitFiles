...
```ats
function RLCPhaseAngleTest(Name: string; Pin1: tpin; Pin2: tpin; Minimum: real; Maximum: real): boolean;  tests rlccombinations;
```

## Call pattern

```ats
RLCPhaseAngleTest('Name', "Pin1", "Pin2", <Minimum>, <Maximum>);
```

## Description

The function tests the phase angle of a RLC combination.
...
```ats
RLCPhaseAngleTest('RLC 1 PA', "Pin1", "Pin2", 70, 80);
```
...
| `RES_PAmin` | `real` | Parameter: Minimum phase angle in degree |
| `RES_PAmax` | `real` | Parameter: Maximum phase angle in degree |
| `RES_Value` | `real` | Measured value in degree |
...
`MeasureRLC`, `RLCDissipationTest`, `RLCImpedanceAbsoluteTest`, `RLCImpedanceImaginaryTest`, `RLCImpedanceRealTest`, `RLCParallelCapacitanceTest`, `RLCParallelInductanceTest`, `RLCParallelResistanceTest`, `RLCQualityTest`, `RLCSerialCapacitanceTest`, `RLCSerialInductanceTest`, `RLCSerialResistanceTest`
