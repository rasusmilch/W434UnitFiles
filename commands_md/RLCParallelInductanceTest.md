# RLCParallelInductanceTest

## Declaration

```ats
function RLCParallelInductanceTest(Name: string; Pin1: tpin; Pin2: tpin; Minimum: tinductance; Maximum: tinductance): boolean; tests rlccombinations;
```

## Call pattern

```ats
RLCParallelInductanceTest('Name', "Pin1", "Pin2", <Minimum>mH, <Maximum>mH);
```
...
```ats
RLCParallelInductanceTest('RLC 1 Lpar', "Pin1", "Pin2", 40mH, 60mH);
```
...
`MeasureRLC`, `RLCDissipationTest`, `RLCImpedanceAbsoluteTest`, `RLCImpedanceImaginaryTest`, `RLCImpedanceRealTest`, `RLCParallelCapacitanceTest`, `RLCParallelInductanceTest`, `RLCParallelResistanceTestRLCPhaseAngleTest`, `RLCQualityTest`, `RLCSerialInductanceTest`
